    ...
    for (const event of eventsInSlot) {
      if (event.disciplinaId === disciplinaId) {
        if (
          ...
          [...firstVespertinoSlots, ...secondVespertinoSlots].includes(
            event.startTime,
          )
        ) {
                ...
                Object.values(phaseEvents).forEach((eventArray) => {
                  ...
                  eventsInSlot.forEach((existingEvent) => {
                    const professoresDoEvento =
                      ...
                      Array.isArray(existingEvent.professoresIds)
                        ? existingEvent.professoresIds
            ...
            todosHorariosOutros.forEach((outroHorario) => {
              ...
              const hora1 =
                typeof horarioAtual.hora_inicio === "object"
                  ? horarioAtual.hora_inicio.toString().substring(0, 5)
                  : horarioAtual.hora_inicio;
              const hora2 =
                typeof outroHorario.hora_inicio === "object"
                  ? outroHorario.hora_inicio.toString().substring(0, 5)
              ...
              if (
                ...
                todosConflitos.push({
                  id: conflictId,
                  professor:
                    professores.find((p) => p.codigo === profId)?.name ||
                    profId,
                  ...
                  diaNome:
                    daysOfWeek.find(
                      (d) =>
                        dayToNumber[d.id] === parseInt(horarioAtual.dia_semana),
                    )?.title || `Dia ${horarioAtual.dia_semana}`,
                  horario1: {
                    ...horarioAtual,
                    disciplinaNome:
      ...
      // Se não tem disciplinaId, tentar encontrar pelo title (compatibilidade com eventos antigos)
        ...
        if (disciplina) {
          ...
          const partialMatch = uniqueDisciplinas.find(
            (d) =>
              d.nome.toLowerCase().includes(event.title.toLowerCase()) ||
              event.title.toLowerCase().includes(d.nome.toLowerCase()),
          );
          if (partialMatch) {
      ...
      if (
        ...
        if (
          ...
          if (timeIndex >= 5) {
            ...
            if (morningProfessoresIds && morningProfessoresIds.length > 0) {
        ...
        if (autoSelectedProfessoresIds.length === 0 && selectedPhase) {
          const existingProfessoresIds = getDisciplinaProfessoresFromSamePhase(
            foundDisciplinaId,
            selectedPhase,
            events,
          );
          if (existingProfessoresIds && existingProfessoresIds.length > 0) {
      ...
      setProfessorAutoSelected(
        autoSelectedProfessoresIds.length > 0 &&
          JSON.stringify(autoSelectedProfessoresIds) !==
            JSON.stringify(
              event.professoresIds || [event.professorId].filter(Boolean),
            ),
      );
        ...
        <Stack spacing={3}>
              ...
              >
                <Box>
                  ...
                  <Typography variant="caption" color="textSecondary">
                    {option.codigo} {option.id ? `(ID: ${option.id})` : ""}
                  </Typography>
                </Box>
              </Box>
            )}
              ...
              <Box sx={{ mb: 2 }}>
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                  {professoresIds.map((profId) => {
                    ...
                    const professorEmConflito = conflitosTempoRealLocal.some(
                      (conflito) =>
                        conflito.professor.includes(professor?.name) ||
                        String(conflito.horario1.codigo_docente) ===
                          String(profId) ||
                        String(conflito.horario2.codigo_docente) ===
                          String(profId),
                    );
                      ...
                      >
                        <Chip
                          ...
                          onDelete={() => {
                            ...
                            const newProfessoresIds = professoresIds.filter(
                              (id) => id !== profId,
                            );
                          ...
                          size="medium"
                          sx={{
                            backgroundColor: professorAutoSelected
                              ? "#4caf50"
                              : undefined,
                            "& .MuiChip-deleteIcon": {
                              color:
          ...
          {professorAutoSelected && (
            <Alert severity="success" variant="outlined" sx={{ mt: 1 }}>
              <Typography variant="caption">
                ✓ Professores preenchidos automaticamente com base em disciplina
                já cadastrada
              </Typography>
            </Alert>
          )}
          ...
          <FormControlLabel
            ...
            label={
              <Box>
                ...
                <Typography variant="caption" color="textSecondary">
                  Marque esta opção se este horário pode ter sobreposição com
                  outros do mesmo professor
                </Typography>
              </Box>
            }
          />
          ...
          {event && (
            ...
            >
              <Typography variant="body2" color="textSecondary">
                ...
                <br />
                <strong>Duração:</strong> {getTotalMinutes()} minutos (
          ...
          >
            ...
            <Button
              ...
              disabled={!disciplinaId || professoresIds.length === 0}
            >
              {conflitosTempoRealLocal.length > 0
...
const ConflitosModal = ({ open, onClose, conflitos, professores }) => {
  const formatarHorario = (inicio, duracao) => {
    ...
    return `${horarioNormalizado} - ${fimHoras
      .toString()
      .padStart(2, "0")}:${fimMinutosRestantes.toString().padStart(2, "0")}`;
  };
        ...
        {conflitos.length === 0 ? (
          ...
          <Box>
            <Alert severity="warning" sx={{ mb: 2 }}>
              <strong>Atenção:</strong> Foram detectados {conflitos.length}{" "}
              conflito(s) de horários. Os professores abaixo têm aulas
              sobrepostas em diferentes semestres.
            </Alert>
              ...
              ([nomeProf, conflitosProf]) => (
                <Box key={nomeProf} sx={{ mb: 3 }}>
                        ...
                        <ListItemText
                          ...
                          secondary={
                            <Box sx={{ mt: 1 }}>
                              ...
                              >
                                <strong>Conflito 1:</strong>{" "}
                                {conflito.horario1.disciplinaNome}
                                <br />
                              ...
                              <Typography variant="body2">
                                <strong>Conflito 2:</strong>{" "}
                                {conflito.horario2.disciplinaNome}
                                <br />
                                ...
                                <br />
                                {formatarHorario(
                                  conflito.horario2.hora_inicio,
                                  conflito.horario2.duracao,
                                )}
                              </Typography>
                            </Box>
                          }
                  ...
                  {Object.keys(conflitosAgrupados).length > 1 &&
                    Object.keys(conflitosAgrupados).indexOf(nomeProf) <
                      Object.keys(conflitosAgrupados).length - 1 && (
                      <Divider sx={{ my: 2 }} />
                    )}
                </Box>
  ...
  const getProfessoresInfo = () => {
    ...
    if (event.professoresIds && Array.isArray(event.professoresIds)) {
      professoresList = event.professoresIds
        .map((profId) =>
          professores.find((p) => p.codigo === profId || p.id === profId),
        )
        .filter(Boolean);
    } else if (event.professorId) {
      const professor = professores.find(
        (p) => p.codigo === event.professorId || p.id === event.professorId,
      );
      if (professor) {
          ...
          {professoresInfo
            ...
            .map((professor, index) => {
              ...
              const professorEmConflito = conflitosDoEvento.some(
                (conflito) =>
                  String(conflito.codigoProfessor) === String(professor.codigo),
              );
        ...
        "&:hover": {
          backgroundColor: isDragOver
            ? theme.palette.mode === "dark"
              ? "rgba(144, 202, 249, 0.16)"
              : "#e3f2fd"
            : timeSlotsNoturno.includes(time) && !isValidStartTimeNoturno(time)
              ? theme.palette.mode === "dark"
        ...
        >
          ...
          {timeSlots.map((time, index) => {
            ...
            const isFirstVespertino =
              temMultiplosTurnos && time === timeSlotsVespertino[0];
            ...
            {timeSlots.map((time, index) => {
              ...
              return (
                <TimeSlot
                  ...
                  events={events}
                  onDropEvent={(eventData, dayId, time) =>
                    onDropEvent(eventData, dayId, time, phaseNumber)
                  }
                  onAddEvent={(dayId, time) =>
    ...
    try {
      ...
      todosHorarios.forEach((horario) => {
        ...
        if (eventoId) {
          ...
          const prioridade =
            horario.tipo === "novo" ? 3 : horario.tipo === "temporario" ? 2 : 1;
          ...
          for (let j = i + 1; j < horariosNoDia.length; j++) {
            ...
            if (horariosSeOverlapam(h1, h2)) {
              ...
              conflitos.push({
                id: conflictId,
                professor:
                  professores.find((p) => p.codigo === codigoProfessor)?.name ||
                  codigoProfessor,
                codigoProfessor,
                dia: dia,
                diaNome:
                  daysOfWeek.find((d) => dayToNumber[d.id] === parseInt(dia))
                    ?.title || `Dia ${dia}`,
                horario1: {
                  ...h1,
                  disciplinaNome:
            ...
            eventsInSlot.forEach((event) => {
              ...
              if (event.disciplinaId && hasProfessores) {
                ...
                if (
                  ...
                  event.professoresIds.forEach((professorId, index) => {
      ...
      if (error.response?.status === 404) {
        ...
        setLoadError(
          "Erro ao carregar ofertas. " +
            (error.response?.data?.message || "Verifique a conexão."),
        );
      } else {
        setLoadError(
          error.response?.data?.message || "Erro ao carregar dados do banco",
        );
      }
      setEvents({});
      setOriginalHorarios([]);
              ...
              if (event.professoresIds && Array.isArray(event.professoresIds)) {
                event.professoresIds.forEach((professorId, index) => {
                  ...
                  currentHorarios.push({
                    ...
                    dia_semana: dayToNumber[event.dayId],
                    hora_inicio: normalizeTimeFromDB(event.startTime),
          ...
          const updatedEvents = eventArray.map((event) => {
            if (event.id === eventData.id) {
              ...
              const updatedEvent = {
                ...
                dia_semana:
                  dayToNumber[eventData.dayId] || eventData.dia_semana,
                ano: ano,
                semestre: semestre,
                fase: selectedPhase,
    ...
    <Box sx={{ padding: 2 }}>
        ...
        >
          ...
          >
            ...
            <Badge badgeContent={conflitosHorarios.length} color="error">
              <Button
                variant={
                  conflitosHorarios.length > 0 ? "contained" : "outlined"
                }
            ...
            <Button
              ...
              startIcon={
                savingHorarios ? <CircularProgress size={20} /> : <SaveIcon />
              }
              sx={{
          ...
          >
            ...
            >
              ...
              <Select
                ...
                startAdornment={
                  loadingCursos && <CircularProgress size={16} sx={{ mr: 1 }} />
                }
              >
                {cursos.map((curso) => (
            ...
            >
              ...
              >
                ...
                {anosSemestres.length === 0 && !loadingAnosSemestres && (
                  <MenuItem disabled>Nenhum ano/semestre cadastrado</MenuItem>
                )}
              </Select>
            </FormControl>
            ...
            return (
              <Alert
                ...
                sx={{
                  ...
                  borderLeft: `4px solid ${
                    anoSemestreAtual.publicado ? "#4caf50" : "#2196f3"
                  }`,
                }}
              ...
              >
                ...
                >
                  <Typography variant="body2" fontWeight="bold">
                    Status dos Horários:{" "}
                    {anoSemestreAtual.publicado ? "PUBLICADOS" : "RASCUNHO"}
                  </Typography>
                </Box>
      ...
      {errorCursos && (
        <Alert
          ...
          sx={{ mb: 2 }}
          action={
            <Button color="inherit" size="small" onClick={fetchCursos}>
              Tentar novamente
            </Button>
          }
        >
      ...
      {errorAnosSemestres && (
        <Alert
          ...
          sx={{ mb: 2 }}
          action={
            <Button color="inherit" size="small" onClick={fetchAnosSemestres}>
              Tentar novamente
            </Button>
          }
        >
      ...
      {!loadingCursos && cursos.length === 0 && !errorCursos && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          Nenhum curso vinculado ao usuário '{getCurrentUserId()}'. Solicite
          acesso aos cursos necessários.
        </Alert>
      )}
      ...
      {errorOfertas && (
        <Alert
          ...
          sx={{ mb: 2 }}
          action={
            <Button color="inherit" size="small" onClick={fetchOfertas}>
              Tentar novamente
            </Button>
          }
        >
        ...
        <>
          ...
          {(() => {
            ...
            if (
              ...
              return (
                <Alert severity="info" sx={{ mb: 2 }}>
                  Nenhum horário encontrado para {selectedAnoSemestre.ano}/
                  {selectedAnoSemestre.semestre}º semestre.
                  <br />
                  Comece criando horários clicando duas vezes nas células da
                  grade.
                </Alert>
              );
            }
              ...
              if (fasesDisponiveis.length === 0) {
                return (
                  <Alert severity="error" sx={{ mb: 2 }}>
                    ...
                    {selectedAnoSemestre.ano}/{selectedAnoSemestre.semestre}º
                    semestre.
                    <br />
                    <Typography
                      variant="body2"
                    ...
                    <Typography
                      ...
                      sx={{ mt: 0.5 }}
                    >
                      1. Cadastre ofertas de fases via API:{" "}
                      <code>POST /api/ofertas</code>
                      <br />
          ...
          {(() => {
            ...
            if (
              ...
              return (
                <Alert severity="success" sx={{ mb: 2 }}>
                  Exibindo {fasesDisponiveis.length} fase(s) conforme ofertas
                  cadastradas: {fasesDisponiveis.join(", ")}ª
                  <br />
                  <Typography variant="caption" display="block">
                    Para adicionar mais fases, cadastre ofertas adicionais na
                    API.
                  </Typography>
                </Alert>
              );
          ...
          {selectedCurso &&
            ...
            (() => {
              ...
              if (
                anoSemestreAtual &&
                (anoSemestreAtual.inicio || anoSemestreAtual.fim)
              ) {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      Período Letivo: {selectedAnoSemestre.ano}/
                      {selectedAnoSemestre.semestre}º Semestre
                    </Typography>
                    <Box
                      sx={{
                    ...
                    >
                      ...
                      >
                        ...
                        <Typography variant="body2">
                          {formatarData(anoSemestreAtual.inicio)}
                        </Typography>
                      </Box>
                      <Box
                        sx={{
                      ...
                      >
                        ...
                        <Typography variant="body2">
                          {formatarData(anoSemestreAtual.fim)}
                        </Typography>
                      </Box>
                    </Box>
                  </Box>
          ...
          <Box sx={{ mt: 3, textAlign: "center" }}>
            <Typography
              ...
              sx={{ mb: 1 }}
            >
              Dica: Clique duplo para adicionar • Arraste para mover • Use a
              alça inferior para redimensionar • Clique para editar • Botão X
              para remover
            </Typography>
            <Typography variant="caption" color="primary" fontWeight="bold">
              {(() => {
                ...
                const statusPublicacao = anoSemestreAtual?.publicado
                  ? "Publicado"
                  : "Rascunho";

                let statusText = `Status (${
                  selectedCurso ? `${selectedCurso.nome} - ` : ""
                }${selectedAnoSemestre.ano}/${
                  selectedAnoSemestre.semestre
                ...
                if (changes.total > 0) {
                  const changeDetails = [];
                  if (changes.added > 0)
                    changeDetails.push(`${changes.added} novo(s)`);
                  if (changes.modified > 0)
                    changeDetails.push(`${changes.modified} editado(s)`);
                  if (changes.removed > 0)
                    changeDetails.push(`${changes.removed} removido(s)`);
            ...
            >
              Horários sem disciplina ou professor não podem ser salvos no
              sistema
            </Typography>
          </Box>
        </>
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body1" sx={{ mb: 2 }}>
            Você possui <strong>{getChangesCount().total} mudança(s)</strong>{" "}
            que ainda não foram sincronizadas com o banco de dados.
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Ao recarregar, todas as mudanças locais serão perdidas e os dados
            serão substituídos pelos do banco.
          </Typography>
